...
# Lazy-load for faster startup - will initialize on first use
# Set at module level to cache after first initialization
embedding_model = None
negotiation_collection = None

def _ensure_initialized():
    """Lazy initialization - only load when first needed"""
    global embedding_model, negotiation_collection
    if embedding_model is None or negotiation_collection is None:
        embedding_model, negotiation_collection = initialize_rag_system()
    return embedding_model, negotiation_collection
